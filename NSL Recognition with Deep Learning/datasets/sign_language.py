...
def load_data(container_path='datasets/nsl_alphabets', test_split=0.2, seed=0):
    """
    Loads sign language dataset.
    """
    folders = []
    for folder in sorted(listdir(container_path)):
        check = folder.find("Letter")
        if check != -1:
    ...
    for label, folder in enumerate(folders):
        ...
        images = [join(folder_path, file)
                     for file in sorted(listdir(folder_path))]
...
def path_to_tensor(img_path, size=64):
